...
def generate_graph_with_constraints(rooms, min_path_length, max_path_length):
    ...
    for room in rooms:
        ...
        for connected_room in additional_connections:
            graph.add_edge(room, connected_room)
...
def generate_random_permutation(rooms, with_constraints=True, max_relax_attempts=3):
    ...
    ground_truth_connections = {node: list(neighbors) for node, neighbors in graph.adjacency()}
    ...
    for room, connected_rooms in ground_truth_connections.items():
        new_connections = [connected_room for connected_room in connected_rooms if (room, connected_room) not in mentioned_pairs and (connected_room, room) not in mentioned_pairs]
        if new_connections:
            description += f"{room} is connected to " + join_with_and(new_connections) + ".\n"
            mentioned_pairs.update((room, connected_room) for connected_room in new_connections)
            mentioned_pairs.update((connected_room, room) for connected_room in new_connections)
...
if __name__ == "__main__":
    create_test_dataset("test_dataset_paths_unweighted.json")
